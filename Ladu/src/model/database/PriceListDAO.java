...
public class PriceListDAO {
	...
	public List<PriceList> findAll() {
		...
		ResultSet result = DBConnection
				.execute("SELECT price_list, price_list_status_type_fk, default_discount_xtra, date_from, date_to, note FROM price_list ");
		if (result == null) {
		...
		try {
			while (result.next()) {
				...
				p.setDefaultDiscountXtra(result
						.getLong("default_discount_xtra"));
				p.setPriceListStatusType(findStatusType(result
						.getInt("price_list_status_type_fk")));
	...
	public PriceListStatusType findStatusType(int status) {
		ResultSet result = DBConnection
				.execute("SELECT price_list_status_type, type_name FROM price_list_status_type WHERE price_list_status_type="
						+ status);
		...
		try {
			if (!result.next()) {
				...
				s.setPriceListStatusType(result
						.getLong("price_list_status_type"));
	...
	public PriceListStatusType findStatusType(String status) {
		ResultSet result = DBConnection
				.execute("SELECT price_list_status_type, type_name FROM price_list_status_type WHERE type_name='"
						+ status + "'");
		PriceListStatusType s = new PriceListStatusType();
		try {
			if (!result.next()) {
	...
	public void createNewPriceList(PriceList priceList, long created_by) {
		...
		try {
			...
			statement.setLong(1, priceList.getPriceListStatusType()
					.getPriceListStatusType());
			statement.setLong(2, priceList.getDefaultDiscountXtra());
			statement.setDate(3, new java.sql.Date(priceList.getDateFrom()
					.getTime()));
			...
			System.out.println(e.getMessage());
			logger.error("PriceListDAO.createNewPriceList() : "
					+ e.getMessage());
		}
	}
	...
	public PriceList findById(int id) {
		ResultSet result = DBConnection
				.execute("SELECT * FROM price_list WHERE price_list=" + id);
		PriceList p = new PriceList();
		try {
			if (!result.next()) {
				return null;
	...
	public void deletePriceList(int id) {
		...
		try {
			PreparedStatement statement = connection
					.prepareStatement("DELETE FROM price_list WHERE price_list="
							+ id);
			statement.execute();
			statement.close();
			connection.close();
	...
	private void deleteAllPriceListItems(int price_list) {
		...
		try {
			PreparedStatement statement = connection
					.prepareStatement("DELETE FROM item_price_list WHERE price_list_fk="
							+ price_list);
			statement.execute();
			statement.close();
			connection.close();
		} catch (SQLException e) {
			logger.error("PriceListDAO.deleteAllPriceListItems() : "
					+ e.getMessage());
		}
	}
	...
	private void deleteAllPriceListCustomers(int price_list) {
		...
		try {
			PreparedStatement statement = connection
					.prepareStatement("DELETE FROM customer_price_list WHERE price_list_fk="
							+ price_list);
			statement.execute();
			statement.close();
			connection.close();
		} catch (SQLException e) {
			logger.error("PriceListDAO.deleteAllPriceListCustomers() : "
					+ e.getMessage());
		}
	}
	...
	public List<String> findOtherStatusTypes(String status) {
		...
		ResultSet result = DBConnection
				.execute("SELECT DISTINCT type_name FROM price_list_status_type WHERE NOT type_name='"
						+ status + "'");
		if (result == null) {
			return null;
		}
		try {
			...
			logger.error("PriceListDAO.findOtherStatusTypes() : "
					+ e.getMessage());
			return null;
		}
	}
	...
	public List<CustomerM> searchCustomer(String name) {
		...
		ResultSet result = DBConnection
				.execute("SELECT subject_id ,subject_name, subject_type FROM"
						+ "(SELECT customer AS subject_id, 'isik'  AS subject_type, to_char(first_name || ' ' || last_name) AS subject_name "
						+ " FROM person INNER JOIN customer ON person=subject_fk WHERE subject_type_fk=1 AND UPPER(last_name) LIKE UPPER('"
						+ name
						+ "%')"
						+ "UNION SELECT customer AS subject_id, 'ettevote'  AS subject_type, to_char( name) AS subject_name FROM enterprise INNER JOIN "
						+ "customer ON enterprise=subject_fk WHERE subject_type_fk=2 AND UPPER(name) LIKE UPPER('"
						+ name + "%' )) ");
	...
	public List<CustomerM> findCustomersById(int price_list) {
		...
		return list;
		// OracleResultSet result = (OracleResultSet)
		// DBConnection.execute("SELECT get_clients("+price_list+") from DUAL");
		// if (result == null) {
		// return null;
		// }
		// try {
	...
	public void deleteCustomer(int customer, int price_list) {
		...
		try {
			PreparedStatement statement = connection
					.prepareStatement("DELETE FROM customer_price_list WHERE price_list_fk="
							+ price_list + " AND customer_fk=" + customer);
			statement.execute();
			statement.close();
			connection.close();
		} catch (SQLException e) {
	...
	public List<ItemM> findItemsById(int price_list) {
		...
		ResultSet result = DBConnection
				.execute("SELECT P.item_price_list, I.item, I.name, I.sale_price, P.discount_xtra, P.sale_price AS price_list_sale_price FROM item  I INNER JOIN item_price_list  P ON I.item=P.item_fk WHERE P.price_list_fk="
						+ price_list + " ORDER BY item");
		if (result == null) {
	...
	public List<ItemM> searchItem(String itemName) {
		List<ItemM> list = new LinkedList<ItemM>();
		ResultSet result = DBConnection
				.execute("SELECT item, name FROM item WHERE UPPER(name) LIKE UPPER('"
						+ itemName + "%') ORDER BY item");
	...
	private void calculateSalePrice(int item, int price_list) {
		...
		try {
			...
			logger.error("PriceListDAO.calculateSalePrice() : "
					+ e.getMessage());
		}
	}
